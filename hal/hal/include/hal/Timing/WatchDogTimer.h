 ...
 * The external watchdog is kicked only if the KickCounter reaches the input parameter N.
 * If the watchdog is kicked, the KickCounter is reset.
 * @param N Number of times this function is called before the watchdog is kicked.
 */
...
/*!
 * Force kicks the watchdog if the last kick of the watchdog was more than 10 milliseconds ago
 */
 ...
 */
void WDT_startWatchdogKickTask(portTickType kickInterval, Boolean toggleLed1);

/*!
 * Starts a low-priority FreeRTOS task that kicks the watchdog at the specified interval.
 */
void WDT_stopWatchdogKickTask();
